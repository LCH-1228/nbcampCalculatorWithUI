...
class AddOperation: Operation {
    ...
    // Operation 프로토콜에서 정의된 calculate의 구현부로 입력된 두 파라미터의 타입이 모두 BinaryInteger로 동일한 경우에 더하기 연산
    public func calculate<T: BinaryInteger>(_ firstNumber: T, _ secondNumber: T) throws -> Double {
        return Double(firstNumber + secondNumber)
    }
    
    // Operation 프로토콜에서 정의된 calculate의 구현부로 입력된 두 파라미터의 타입이 모두 FloatingPoint로 동일한 경우에 더하기 연산
    ...
    // Operation 프로토콜에서 정의된 calculate의 구현부로 입력된 첫번째 파라미터와 두번째 파라미터가 FloatingPoint인 경우
